...
namespace GuacamoleClient.WinForms
{
    internal sealed class AddEditServerForm : Form
    {
        ...
        private readonly CheckBox _chkIgnoreCert = new CheckBox { Text = "Ignore certificate errors (unsafe)", AutoSize = true };

        private readonly Button _btnSave = new Button { Text = "Save", DialogResult = DialogResult.None, Anchor = AnchorStyles.Bottom | AnchorStyles.Right };
        ...
        public AddEditServerForm(GuacamoleSettingsManager manager, GuacamoleServerProfile? editing, bool isFirstProfile)
        {
            ...
            Text = editing == null ? "Add Guacamole server" : "Edit Guacamole server";
            ...
            layout.Controls.Add(new Label { Text = "Server URL", AutoSize = true, TextAlign = ContentAlignment.MiddleLeft }, 0, 0);
            layout.Controls.Add(_txtUrl, 1, 0);
            layout.SetColumnSpan(_txtUrl, 2);

            layout.Controls.Add(new Label { Text = "Display name (optional)", AutoSize = true, TextAlign = ContentAlignment.MiddleLeft }, 0, 1);
            layout.Controls.Add(_txtName, 1, 1);
            layout.SetColumnSpan(_txtName, 2);

            layout.Controls.Add(new Label { Text = "Color scheme", AutoSize = true, TextAlign = ContentAlignment.MiddleLeft }, 0, 2);
            layout.Controls.Add(_cmbColor, 1, 2);
            layout.Controls.Add(_pnlColorPreview, 2, 2);

            layout.Controls.Add(new Label { Text = "Custom color (hex)", AutoSize = true, TextAlign = ContentAlignment.MiddleLeft }, 0, 3);
        ...
        private async System.Threading.Tasks.Task SaveAsync()
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show(this, "Server URL is required.", "Validation", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                if (!GuacamoleUrlAndContentChecks.IsValidUrlAndAcceptedScheme(url))
                {
                    MessageBox.Show(this, "The URL is invalid. Please use http:// or https://.", "Validation", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                {
                    MessageBox.Show(this, "This URL is already registered.", "Validation", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                if (!ColorValueResolver.TryResolveToHex(colorValue, out var _))
                {
                    MessageBox.Show(this, "Invalid color value. Choose a palette color or enter a hex value like #A1B2C3.", "Validation", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                {
                    var msg = "This color is already assigned to: " + string.Join(", ", collisions) + "\n\nDo you want to use it anyway?";
                    if (MessageBox.Show(this, msg, "Color already in use", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) != DialogResult.Yes)
                        return;
                }
                ...
                if (!ok)
                {
                    MessageBox.Show(this,
                        "The URL does not respond with an Apache Guacamole start page (or is not reachable).\n\nExample: https://remote.example.com/guacamole/",
                        "Server test failed",
